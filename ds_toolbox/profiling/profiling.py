    ...
    def inner(func):
        ...
        def wrapper(*args, **kwargs):
            if not isinstance(profiles_path, Path):
                raise TypeError(f"profiles_path must be an instance of <class pathlib.Path>, got {type(profiles_path)}")
            if not isinstance(profile_name, str):
                raise TypeError(f"profile_name must be an instance of <class str>, got {type(profile_name)}")
